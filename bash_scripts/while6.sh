...
echo $myvar && echo $myvar | cat >> ~/testfilelog
    ...
    echo "Looking for thestfile" && echo "Looking for test file at $(date)" | cat >> ~/testfilelog
    cp /home/adrian/TESTFILESTART/testfile ~/testfile
    if [ -f ~/testfilestart ] | [ -f ~/testfile ]
    then
	/home/adrian/Scripts/bash_scripts/while6.sh
    fi
else
